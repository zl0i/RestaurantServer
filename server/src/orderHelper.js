...
async function aggregateMenu(shop_id, menu) {
    let items = await Shop.aggregate([
        ...
        { $unwind: "$items.menu" },
        { $match: { "items.menu.id": { $in: menu.map(item => item.id) }, "items.menu.isEnd": false } },
        { $project: { _id: 1, item_id: "$items.menu.id", cost: "$items.menu.cost" } },
        { $addFields: { count: { $arrayElemAt: [menu.map(item => item.count), { $indexOfArray: [menu.map(item => (item.id)), '$item_id'] }] } } }
    ]);
