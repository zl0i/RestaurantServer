...
describe('POST /orders', () => {
    ...
    test('should create new order', async () => {
        ...
        let verifyUserDataMock = orderHelper.verifyUserData = jest.fn().mockImplementation(() => Promise.resolve());
        let verifyOrderDataMock = orderHelper.verifyOrderData = jest.fn().mockImplementation(() => Promise.resolve())
        let updateUserAddressMock = orderHelper.updateUserAddress = jest.fn().mockImplementation(() => Promise.resolve(user));
        let findShopMock = Shop.findOne = jest.fn().mockImplementation(() => Promise.resolve(shop));
        let calcCostMenuMock = orderHelper.calcCostMenu = jest.fn().mockImplementation(() => Promise.resolve(items));
        let uuidv4Mock = uuidv4.mockReturnValue(order.id);
        let createPaymentMock = yookassa.createPaymentOrder = jest.fn().mockImplementation(() => Promise.resolve(payment));
        let createOrderMock = orderHelper.createOrder = jest.fn().mockImplementation(() => Promise.resolve());
        ...
        expect(createPaymentMock.mock.calls[0])
            .toEqual([total_cost, order.id, 'Ваш заказ']);
    ...
    test('error verify user data', async () => {
        ...
        let verifyUserDataMock = orderHelper.verifyUserData = jest.fn().mockImplementation(() => { throw new Error('error verify user data') });
        let createOrderMock = orderHelper.createOrder = jest.fn().mockImplementation(() => Promise.resolve());
    ...
    test('error verify order data', async () => {
        ...
        let findShopMock = Shop.findOne = jest.fn().mockImplementation(() => Promise.resolve(shop));
        let calcCostMenuMock = orderHelper.calcCostMenu = jest.fn().mockImplementation(() => Promise.resolve(items));
        let verifyOrderDataMock = orderHelper.verifyOrderData = jest.fn().mockImplementation(() => { throw new Error('error verify order data') })
        let createOrderMock = orderHelper.createOrder = jest.fn().mockImplementation(() => Promise.resolve());
    ...
    test('error create payment', async () => {
        ...
        let calcCostMenuMock = orderHelper.calcCostMenu = jest.fn().mockImplementation(() => Promise.resolve(items));
        let uuidv4Mock = uuidv4.mockReturnValue(order.id);
        let createPaymentMock = yookassa.createPaymentOrder = jest.fn().mockImplementation(() => { throw new Error('error create payment') });
        let createOrderMock = orderHelper.createOrder = jest.fn().mockImplementation(() => Promise.resolve());
...
describe("DELETE /orders/:id", () => {
    ...
    test("should delete orders", async () => {
        ...
        for (let i = 0; i < typesOrder.length; i++) {
            let deleteFn = ActiveOrders.deleteOne = jest.fn().mockImplementation(() => Promise.resolve([]));
            let findFn = ActiveOrders.findOne = jest.fn().mockImplementation(() => Promise.resolve(order));
            let order = {
                id: "1",
                status: typesOrder[i]
            };
