...
router.post('/login', async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            'activeOrder': await ActiveOrders.findOne({ user_id: user._id }, { _id: 0, total: 1, datetime: 1, status: 1 }),
...
router.post('/info', async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            'token': user.token,
            'history': user.orders.reverse(),
            'activeOrder': await ActiveOrders.findOne({ user_id: user._id }, { _id: 0, id: 1, total: 1, datetime: 1, status: 1, menu: 1 }),
            'address': user.address
        })
    } catch (error) {
        res.status(401).end();
